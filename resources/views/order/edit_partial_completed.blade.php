                        ...
                        <table id="customer" class="table table-striped table-bordered table-hover table-sm">
                            ...
                            @foreach($order_items as $order_item)
                                 ...
                                 <input type="hidden" name="update_unit_price[]" value="{{$order_item['product_price']}}" id="update_unit_price_{{$order_item['order_item_id']}}" />   
									...
									$uk_avl = $order_item['quantity'];
									 if($order_item['s_from']=='my_stock'){
									 	echo 'MY Stock-'.($order_item['quantity'] - $order_item['dispatch_quantity']);
									 }elseif($order_item['shipping_location_id']!=129){
									 	if($order_item['s_from']=='my_stock'){
											echo 'MY Stock-'.($order_item['quantity'] - $order_item['dispatch_quantity']);
										}else{
											echo 'UK Stock-'.($order_item['quantity'] - $order_item['dispatch_quantity']);
										}
									 }else{
										...
										<ul class="main-menu text-left">
										 ...
										 <?php
												 ...
												 if($showed_dispatch=='0'){
													 ...
													 $shipment2 = App\Shipment::select('shipment_number')->where('id',$shipment_info->shipment_id)->first();
												/*echo 'MY Stock-'.($shipment_info->ship_quantity - $order_item['pending_quantity']).'<br>'; */
												
											$shipment_pending = DB::table('shipment_line')->select('pending_quantity')->where('item_id',$order_item['product_id'])->where('shipment_id',$shipment_info->shipment_id)->first();	
											
											if($shipment_pending->pending_quantity!=0){
												?>
                                                <div class="dropdown">
									   <a style="text-decoration:underline;" href="javascript:void();" class="dropbtn"><span>Pending-{{$order_item['pending_quantity']}}</span></a><div class="dropdown-content">
										<ul class="main-menu text-left">
											<li><pre><h6>Tracking information</h6></pre></li>
											<li><pre>{{$shipment2->shipment_number}} </pre></li>
											<li><pre>Pending Quantity : {{$order_item['pending_quantity']}} </pre></li>
										</ul>
									 </div>
									</div><br />
                                                ...
                                                <?php
														...
														echo 'Damage/Lost-'.$order_item['pending_quantity'].'<br>';
													}
												 }else{
													 echo 'MY Stock-'. ($shipment_info->ship_quantity - $order_item['dispatch_quantity']).'<br>'; 
												 }
									    ...
									    foreach($shipment_list as $shipment_info ){ 
										 ...
										 if($uk_avl>0){
										  ...
										  if($order_item['pending_quantity']>0){
											 $shipment_pending = DB::table('shipment_line')->select('pending_quantity')->where('item_id',$order_item['product_id'])->where('shipment_id',$shipment_info->shipment_id)->first();	
											if($shipment_pending->pending_quantity!=0){
											echo '<span class="badge badge-warning badge-pill">Pending</span>'; 
											}else{
												echo '<span class="badge badge-danger badge-pill">Damage</span>'; 
											}
										 }
										?>
                                        </td>
                                ...
                                <tr>
                                    ...
                                    <th></th>
                                    <th>
                                    @hasrole('Super-Admin')<span class="badge badge-success badge-pill" id="total_local_postage_cost_badge" ondblclick="show_postage_box();">(RM) {{$order[0]->total_local_postage_cost}}</span><input type="number"   name="total_local_postage_cost_input" id="total_local_postage_cost_input" class="form-control" required="required" min="0" data-parsley-min-message="Minimum 1" data-parsley-required-message="Set cost" onblur="set_local_postage_cost(this)"  value="{{$order[0]->total_local_postage_cost}}" style="display:none; width:120px;"/>
                                    @else
                                      <span class="badge badge-success badge-pill" id="total_local_postage_cost_badge" >(RM) {{$order[0]->total_local_postage_cost}}</span>
                                     @endhasrole</th>
                                    <th></th>
                                    <th></th>
                                    <th></th>
